...
(function() {
    ...
    function TemplateDesignerController(ballotTemplateDesignerService, fabric, fabricConstants, $scope, 
        ...
        vm.designerData = angular.extend({bgColor: "#ffffff", borderColor: "#cccccc"}, designerData);
    
        vm.ok = function () {
            if (canvasObjectsAreValid()) {
//                vm.warning = {};
                let ballotTemplateAsGroup = createBallotTemplateGroup();
                ballotTemplateAsGroup.subtype = 'pageArea:' + vm.designerData.view;
                let responseData = {
                ...
                $log.error("Fix errors");
//                vm.warning = {
//                    title: "THE TEMPLATE CANNOT BE SAVED",
//                    message: "Some objects are outside the canvas limits"
//                }
                vm.infoBox = {
                    mode: 'warning',
                    message: 'Some objects are outside the canvas limits.'
        ...
        vm.fontSizes = [{id: 0, value: 8}].concat(fabricConstants.fontSizes);
        ...
        function preventDragoverDefaultAction(e) {
            e = e || event;
            e.preventDefault();
            $log.info("dragover");
        }
        
        function preventDropDefaultAction(e) {
            e = e || event;
            e.preventDefault();
            $log.info("drop");
        }
        ...
        function preventDragAndDropOverBrowserWindow() {
            $log.info("Adding D&D over browser");
            $window.addEventListener("dragover", preventDragoverDefaultAction, false);
            $window.addEventListener("drop", preventDropDefaultAction, false);
        }
        
        function removeDragAndDropOverBrowserListener() {
            $log.info("Removing D&D over browser");
            $window.removeEventListener("dragover", preventDragoverDefaultAction);
            $window.removeEventListener("drop", preventDropDefaultAction);
        }
...
//            replacePlaceholderObjects();
            ...
            var areaBackground = $scope.fabric.addRect({
                left: 0, top: 0, 
                width: vm.designerData.canvas.width, height: vm.designerData.canvas.height,
                fill: vm.designerData.canvas.bgColor, stroke: vm.designerData.canvas.borderColor, 
                strokeWidth: 1, opacity: 1, subtype: 'background', renderToCanvas: false
            });
            ballotTemplateObjects.push(areaBackground);
...
//                    gridColumnsWidth, gridRowsHeight);
            ...
            $scope.fabric.getCanvas().setBackgroundColor(designerData.canvas.bgColor);
            $scope.fabric.getCanvas().renderAll();
            
        };
        
        function canvasObjectsAreValid() {
            let result = true;
            let canvas = $scope.fabric.getCanvas();
            
            canvas.forEachObject(function(obj) {
                if (!obj.excludeFromExport && 
                        obj.get('subtype') !== 'barcode' && 
                        obj.get('subtype') !== 'pageNumber') {
                    if (objectOutsideCanvasLimits(obj, canvas.getWidth(), canvas.getHeight())) {
        ...
        function objectOutsideCanvasLimits(obj, canvasWidth, canvasHeight) {
            if (obj.left < 0 || obj.top < 0 || 
                    obj.left + obj.width > canvasWidth ||
                    obj.top + obj.height > canvasHeight) {
                return true;
            }
        }
        ...
        function ungroup(group) {
            ...
            items.forEach(item => {
                if (item.subtype !== 'background') {
                    $scope.fabric.getCanvas().add(item);
                }
//                if (item.subtype) {
//                    $scope.fabric.getCanvas().add(item);
//                }
        ...
        function setView(viewName) {
            ...
            if (vm.currentView === 'header') {
                ...
                vm.drawElement(vm.templateManager.fixedElements[0]);
                vm.drawElement(vm.templateManager.fixedElements[1]);
        ...
        vm.templateManager.variables = [
            {value: '${election.title.EN}', name: 'Election title (EN)'},
            {value: '${election.title.ES}', name: 'Election title (ES)'},
            {value: '${election.date.EN}', name: 'Election date (EN)'},
            {value: '${election.date.ES}', name: 'Election date (ES)'},
            {value: '${election.date.PT}', name: 'Election date (PT)'}
        ]

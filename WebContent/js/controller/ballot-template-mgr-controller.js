...
//        dataService.getContestClasses().then(result => $log.info(JSON.stringify(result)));
        ...
        function preventDragAndDropDefaultAction(e) {
            e = e || event;
            e.preventDefault();
            if (e.type === "drop") {
        ...
        function preventDragAndDropOverBrowserWindow() {
            $window.addEventListener("dragover", preventDragAndDropDefaultAction, false);
            $window.addEventListener("drop", preventDragAndDropDefaultAction, false);
        }
        
        function removeDragAndDropOverBrowserListener() {
            $window.removeEventListener("dragover", preventDragAndDropDefaultAction);
            $window.removeEventListener("drop", preventDragAndDropDefaultAction);
        }
...
//                        templateType: 'STATIC',
            ...
            vm.ballotAreas[0].components[0].fill = "#cef7fb";
            vm.ballotAreas[0].components[0].stroke = "#cccccc";
        ...
        function openTemplateDesigner(designerData) {
            preventDragAndDropOverBrowserWindow();
            ...
            modalInstance.result.then(function(responseData) {
                ...
                removeDragAndDropOverBrowserListener();
                $log.info("Save");
            }, function() {
                $scope.fabric.setDirty(false);
                removeDragAndDropOverBrowserListener();
        ...
        */
        ...
        vm.verifyLanguageGroupsVariables = verifyLanguageGroupsVariables;
        function verifyLanguageGroupsVariables() {
            let textboxs = [];
            let groupLanguages = ['EN', 'ES'];
            
            let canvasObjects = $scope.fabric.getCanvas().getObjects();
            canvasObjects.forEach(o => {
                textboxs = textboxs.concat(traverseFabricElement(o));
            })
            $log.info(textboxs);
            vm.verifyLanguage = canvasObjects.length;
            
            let rawVariables = extractRawVariables(textboxs);
            $log.info(rawVariables);
            
            let languagesByVariables = groupLanguagesByVariables(rawVariables);
            $log.info(languagesByVariables);
            
            let incompleteVariables = [];
            languagesByVariables.forEach(variable => {
                let missingLanguages = groupLanguages.filter(e => variable.languages.indexOf(e) === -1);
                if (missingLanguages.length > 0) {
                    incompleteVariables.push({
                        'name': variable.name,
                        'missingLanguages': missingLanguages.join(', ')
                    });
                }
            });
            $log.info(incompleteVariables);
            
            // Variables associated to a language that doesn't exists in the current language group
            let leftoverVariables = [];
            languagesByVariables.forEach(variable => {
                let leftoverLanguages = variable.languages.filter(e => groupLanguages.indexOf(e) === -1);
                if (leftoverLanguages.length > 0) {
                    leftoverVariables.push({
                        'name': variable.name,
                        'leftoverLanguages': leftoverLanguages.join(', ')
                    });
                }
            });
            $log.info(leftoverVariables);
        }
        
        function traverseFabricElement(fabricElement) {
            let textboxs = [];
            if (fabricElement.excludeFromExport || (fabricElement.subtype && fabricElement.subtype.toLowerCase().includes('placeholder'))) {
                $log.info("Excluding: " + fabricElement.id);
                return textboxs;
            }
            let fabricObjects = fabricElement.getObjects();
            if (fabricObjects && fabricObjects.length > 0) {
                fabricObjects.forEach(o => {
                    if (o.type === 'group') {
                        textboxs.concat(traverseFabricElement(o));
                    } else {
                        if (o.type === 'textbox') {
                            textboxs.push(o.text);
                        }
                    }
                })
            }
        ...
        function extractRawVariables(textboxs) {
            let variables = [];
            textboxs.forEach(textbox => {
                let variableStart = 0;
                let variableEnd = 0;
                do {
                    variableStart = textbox.indexOf('${');
                    variableEnd = textbox.indexOf('}') + 1;
//                  $log.info('variableStart: ' + variableStart + ', variableEnd: ' + variableEnd);
                    if (variableStart !== -1 && variableEnd !== 0) {
                        $log.info(textbox);
                        let variable = textbox.substring((variableStart + 2), (variableEnd - 1));
//                      $log.info(variable + ", " + (variableStart + 2) + ", " + (variableEnd - 1));
                        variables.push(variable);                		
                    }
                    textbox = textbox.substring(0, variableStart) + textbox.substring(variableEnd);
                } while (variableStart !== -1 && variableEnd !== 0);
            });
            return variables;
        }
        
        function groupLanguagesByVariables(rawVariables) {
            let languagesByVariables = [];
            rawVariables.forEach(variable => {
                let lastDot = variable.lastIndexOf('.');
                let variableName = variable.substring(0, lastDot);
                let language = variable.substring(lastDot + 1);
                $log.info(variableName + ' - ' + language);

                let pos = -1;
                languagesByVariables.filter(function(e, index) {
                ...
                if (pos === -1) {
                    languagesByVariables.push({
                        'name': variableName,
                        'languages': [language]
                    });
                } else {
                    languagesByVariables[pos].languages.push(language);
                }
            });
            return languagesByVariables;
        }

...
//                    ballotOptionGroup._updateObjectsCoords();
        ...
        function fillContestTemplate(template, availableArea) {
            ...
            var templateInitialWidth = template.width;
            ...
            adjustBallotTemplateHeadersWidth(template, templateInitialWidth, template.width - 3);
        ...
        function adjustBallotTemplateHeadersWidth(template, initialWidth, newWidth) {
            var delta = newWidth - initialWidth;
            template.forEachObject(function(obj) {
                if (!obj.subtype) {
                    obj.width = newWidth;
                    //obj.left = - (newWidth / 2);
                    //obj.left = obj.left - (delta / 2);
                }
        ...
        function adjustBallotTemplateHeadersWidthX(template, newWidth) {
            template.forEachObject(function(obj) {
                if (!obj.subtype) {
                    obj.width = newWidth;
                    obj.left = - (newWidth / 2);
...
//                            angular.element('#load-ballot-config-modal').modal('show');
        ...
        function associateContestAndRenderCandidates(fabricObject) {
            ...
            modalInstance.result.then(function(userSelections) {
                ...
                var finalJson = JSON.stringify(fabricObject.toObject(ballotDesignerCustomProperties));
                $log.info("Debug: " + finalJson);
            }, function() {
                $scope.fabric.setDirty(false);
            });
        };
...
//            angular.element('#delete-ballot-style-modal').modal('show');
        ...
        function viewPreviousCanvas() {
            ...
            $log.info("Elapsed time (view previous canvas): " + (performance.now() - startMilli));
        }
        ...
        function viewNextCanvas() {
            ...
            storeCurrentCanvas();
            loadCanvasPage(vm.canvas.currentPage + 1);
            $log.info("Elapsed time (view next canvas): " + (performance.now() - startMilli));
        }
